...
Following we will discribe, how to use Kubernetes for Sakuli E2E testing and monitoring. First go the folder of the Kubernetes conf:
...
There you will find the following kubernetes config example `kubernetes.sakuli.example.pod.run.yaml`:

[source, yaml]
----
include:: ../../../../../../docker/kubernetes/kubernetes.sakuli.example.pod.run.yaml[]
----

...
The running containers are accessible with VNC (default password: _`sakuli`_) by:
...
Also for Sakuli test writen in Java and executed through https://maven.apache.org[Maven], we provide to preconfigured docker images: https://hub.docker.com/search/?isAutomated=0&isOfficial=0&page=1&pullCount=0&q=sakuli-java&starCount=0[consol/sakuli-xxx-xxx-java]. More information about how to write a Java based Sakuli test, see <<sakuli-java-dsl>>. Now take a look at the important lines in the _Dockerfile_ which define how the container will start:
...
//TODO TS update
* link:openshift.md[Usage of Sakuli containers in OpenShift]
* link:kubernetes.md[Usage of Sakuli containers in Kubernetes]
* link:omd-labs-sakuli.md[Usage of Sakuli containers together with OMD-Labs Docker Images]
* Publications:

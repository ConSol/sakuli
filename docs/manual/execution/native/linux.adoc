...
If you want to *update* Sakuli, see the link:upgrade-process.md[upgrade process] documentation.
...
* Other optional steps see link:./troubleshooting-tuning-sakuli-client.md#desktop-tuning["Desktop tuning"].
* 
...
* If you are sitting behing a company proxy, refer to link:./sakuli-additional-settings.md#sahi-behind-a-proxy[Proxy settings].
* Refer to link:./additional-settings.md#browser-configuration[Browser configuration]) for instructions how to register other browsers.
...
Currently, _each_ Sakuli test requires to start a browser, which is not very handy for pure Sikuli GUI tests (=where no browser at all is needed). For that case, use a headless browser like http://phantomjs.org[PhantomJS]. Refer to link:./additional-settings.md#browser-configuration[Browser configuration] for more information.
...
* link:installation-omd.md[Open Monitoring Distribution (OMD)].
* link:forwarder-icinga2.md[Icinga2]

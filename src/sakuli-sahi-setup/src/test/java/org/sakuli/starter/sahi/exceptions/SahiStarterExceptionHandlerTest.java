...
public class SahiStarterExceptionHandlerTest extends BaseTest {
    ...
    public void init() throws Exception {
        ...
        testling = BeanLoader.loadBean(SakuliExceptionHandler.class);
        sahiReport = loaderMock.getSahiReport();
        reset(sahiReport);
        testSuite = BeanLoader.loadBean(TestSuite.class);
        ReflectionTestUtils.setField(testSuite, "exception", null);
        screenActionLoader = BeanLoader.loadBean(ScreenActionLoader.class);
        baseLoader = BeanLoader.loadBean(BaseActionLoaderImpl.class);
        ...
        when(screenActionLoader.getActionProperties()).thenReturn(actionProperties);
        when(screenActionLoader.getSakuliProperties()).thenReturn(sakuliProperties);
        when(baseLoader.getSakuliProperties()).thenReturn(sakuliProperties);
    ...
    protected String getTestContextPath() {
        return "org/sakuli/starter/sahi/exceptions/sahiExceptionHandlerTest-beanRefFactory.xml";
    }
    ...
    public void testHandleSakuliProxyExceptionForTestCases() throws Exception {
        when(screenActionLoader.getCurrentTestCase()).thenReturn(null);
        String testExcMessage = "init-exception";
        ...
        verify(screenActionLoader, never()).getScreenshotActions();
    }

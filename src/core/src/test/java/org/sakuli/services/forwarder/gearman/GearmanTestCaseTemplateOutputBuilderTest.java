 ...
 */
public class GearmanTestCaseTemplateOutputBuilderTest extends BaseTest {
    ...
    private Map<TemplateModelEntityName, Object> getAdditionalModelEntities(AbstractTestDataEntity abstractTestDataEntity) {
        BaseActionLoader baseActionLoader = BeanLoader.loadBaseActionLoader();
        if (TestCase.class.isAssignableFrom(abstractTestDataEntity.getClass())) {
            baseActionLoader.getTestSuite();
            return Collections.unmodifiableMap(new HashMap<TemplateModelEntityName, Object>() {
                private static final long serialVersionUID = 7450706784088252868L;

                {
                    put(TemplateModelEntityName.TEST_SUITE_ID, baseActionLoader.getTestSuite().getId());
                }
            });
        }
        else if (TestCaseStep.class.isAssignableFrom(abstractTestDataEntity.getClass())) {
            return Collections.unmodifiableMap(new HashMap<TemplateModelEntityName, Object>() {
                private static final long serialVersionUID = 2714823199357545464L;

                {
                    put(TemplateModelEntityName.TEST_SUITE_ID, baseActionLoader.getTestSuite().getId());
                    put(TemplateModelEntityName.TEST_CASE_ID, baseActionLoader.getCurrentTestCase().getId());
                }
            });
        }

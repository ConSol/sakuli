...
function main() {
	...
	# copy Sakuli logo
	mkdir_cp /tmp/sakuli/docs/pics/sakuli.png $TMPDIR/local/share/nagios/htdocs/images/logos/

	chown -R demo:demo $TMPDIR

	rsync -avz --recursive {{ role_path }}/files/{{ item }} /omd/sites/demo/
}
